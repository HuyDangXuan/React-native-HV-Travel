...
type Tour = {
  rating?: number;       // avg từ backend
  reviewCount?: number;  // count từ backend
  reviews?: Review[];    // populated reviews (nếu API trả embed)
};
...
export default function ReviewTab({ tour }: { tour: Tour | null }) {
  ...
  const ratingStats = useMemo(() => {
    ...
    const hasBackendAvg = typeof tour?.rating === "number" && typeof tour?.reviewCount === "number";
